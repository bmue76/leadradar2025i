...
export async function GET(
  ...
  const fields = await prisma.formField.findMany({
    where: {
      tenantId: tenantRes.tenant.id,
      formId: form.id,
      isActive: true, // mobile: only active fields
    },
    select: {
      key: true,
      label: true,
  ...
  const mappedFields = fields.map((f: (typeof fields)[number]) => ({
    key: f.key,
    label: f.label,
    type: f.type,
    required: f.required,
  ...
  return jsonOk(req, {
    form: {
      ...
      fields: mappedFields, // ✅ Mobile-friendly: form.fields
    },
    fields: mappedFields, // ✅ backward compatible: fields top-level
  });
}
